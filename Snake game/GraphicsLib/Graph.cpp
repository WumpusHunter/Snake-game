...
namespace Graph_lib {
	...
	*/
	...
	Grid::Grid(Point xy, int cell_w, int cell_h, int h_num, int v_num)
		: grid{}
	{
		if (h_num <= 0 || v_num <= 0)		// Error handling
			throw invalid_argument("Bad Grid: non-positive number of lines");
		// Fill of grid
		for (int h_sz = 0; h_sz < h_num; ++h_sz)          // Horizontal line
			for (int v_sz = 0; v_sz < v_num; ++v_sz)      // Vertical line
				grid.push_back(new Rectangle{ Point{ xy.x + cell_w * h_sz, xy.y + cell_h * v_sz }, cell_w, cell_h });
		add(xy);							// Top-left angle of grid
	}

	// Draws grid and fills it with color, if required
	void Grid::draw_lines() const
	{
		// Draw each cell of grid
	...
	{
		Shape::move(dx, dy);
		// Move each cell of grid
		for (int i = 0; i < grid.size(); ++i)
			grid[i].move(dx, dy);
	}

	// Sets c as color of grid's lines
	void Grid::set_color(Color c)
	{
		Shape::set_color(c);
		// Set c as color of lines to each cell of grid
		for (int i = 0; i < grid.size(); ++i)
			grid[i].set_color(c);
	}

	// Sets c as fill color of grid
	void Grid::set_fill_color(Color c)
	{
		Shape::set_fill_color(c);
		// Set c as fill color to each cell of grid
		for (int i = 0; i < grid.size(); ++i)
			grid[i].set_fill_color(c);
	}

	// Sets ls as line style of grid
	void Grid::set_style(Line_style ls)
	{
		Shape::set_style(ls);
		// Set ls as line style to each cell of grid
		for (int i = 0; i < grid.size(); ++i)
			grid[i].set_style(ls);
	}
	...
	int Grid::width() const
	{
		...
		auto minmax_x = minmax_element(grid.cbegin(), grid.cend(),
			[](const Rectangle* a, const Rectangle* b) { return a->point(0).x < b->point(0).x; });
		auto min_x = *minmax_x.first, max_x = *minmax_x.second;
	...
	int Grid::height() const
	{
		...
		auto minmax_y = minmax_element(grid.cbegin(), grid.cend(),
			[](const Rectangle* a, const Rectangle* b) { return a->point(0).y < b->point(0).y; });
		auto min_y = *minmax_y.first, max_y = *minmax_y.second;
		// Difference between min and max x-coordinates of cells, added to width of cell
